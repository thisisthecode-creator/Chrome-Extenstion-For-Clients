...
// Base URL and API key can be changed in Settings (Seats.aero API URL / API Key).

(function() {
  const DEFAULT_BASE = 'https://seats.aero/partnerapi';
  const DEFAULT_TAKE = 20;
  const CACHE_TTL_MS = 5 * 60 * 1000;

  function getApiBase() {
    return (window.config && window.config.api && window.config.api.seatsAero && window.config.api.seatsAero.baseUrl) || DEFAULT_BASE;
  }
  function getApiKey() {
    const k = window.config?.api?.seatsAero?.key;
