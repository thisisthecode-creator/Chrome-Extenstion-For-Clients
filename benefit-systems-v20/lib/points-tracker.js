...
(function() {
  ...
  class PointsTracker {
      ...
      return await window.errorHandler?.safeAsync(async () => {
        ...
        try {
          ...
          this.entries = entriesArray.map(row => ({
            ...
            pointsPending: row.points_pending !== null && row.points_pending !== undefined ? parseFloat(row.points_pending) : 0,
            pointValue: parseFloat(row.point_value) || 0.0190,
            myValue: row.my_value !== null && row.my_value !== undefined ? parseFloat(row.my_value) : null,
     ...
     * @param {number} entry.points - Amount of points/miles
    ...
    async addEntry(entry) {
      return await window.errorHandler?.safeAsync(async () => {
        ...
        const supabaseData = {
          ...
          points_pending: entry.pointsPending !== null && entry.pointsPending !== undefined ? parseFloat(entry.pointsPending) : null,
          point_value: pointValue,
          my_value: entry.myValue !== null && entry.myValue !== undefined ? parseFloat(entry.myValue) : null,
        ...
        if (response && (Array.isArray(response) ? response[0] : response)) {
          ...
          const newEntry = {
            ...
            points: parseFloat(responseData.points) || 0,
            pointsPending: responseData.points_pending !== null && responseData.points_pending !== undefined ? parseFloat(responseData.points_pending) : 0,
            pointValue: parseFloat(responseData.point_value) || 0.0190,
            myValue: responseData.my_value !== null && responseData.my_value !== undefined ? parseFloat(responseData.my_value) : null,
    ...
    async updateEntry(id, updates) {
      return await window.errorHandler?.safeAsync(async () => {
        ...
        // Always update points if provided
        if (updates.points !== undefined) {
          supabaseData.points = parseFloat(updates.points) || 0;
        } else {
          supabaseData.points = parseFloat(entry.points) || 0;
        }
        
        // Always update points_pending if provided
        if (updates.pointsPending !== undefined) {
          supabaseData.points_pending = updates.pointsPending !== null && updates.pointsPending !== undefined ? parseFloat(updates.pointsPending) : null;
        } else {
          supabaseData.points_pending = entry.pointsPending !== null && entry.pointsPending !== undefined ? parseFloat(entry.pointsPending) : null;
        }
        ...
        // Always update my_value if provided
        if (updates.myValue !== undefined) {
          supabaseData.my_value = updates.myValue !== null && updates.myValue !== undefined ? parseFloat(updates.myValue) : null;
        } else {
          supabaseData.my_value = entry.myValue !== null && entry.myValue !== undefined ? parseFloat(entry.myValue) : null;
        }
        ...
        // Update local cache
        if (updates.points !== undefined) entry.points = parseFloat(updates.points) || 0;
        if (updates.pointsPending !== undefined) entry.pointsPending = updates.pointsPending !== null && updates.pointsPending !== undefined ? parseFloat(updates.pointsPending) : 0;
        if (updates.programName !== undefined) {
        ...
        if (updates.myValue !== undefined) {
          entry.myValue = updates.myValue !== null && updates.myValue !== undefined ? parseFloat(updates.myValue) : null;
        }
        ...
        if (!window.httpClient) {
          ...
          headers: {
            ...
            if (response && response[0]) {
              ...
              entry.points = parseFloat(response[0].points) || 0;
              entry.pointsPending = response[0].points_pending !== null && response[0].points_pending !== undefined ? parseFloat(response[0].points_pending) : 0;
              entry.programName = response[0].program_name;
              entry.pointValue = parseFloat(response[0].point_value) || 0.0190;
              entry.myValue = response[0].my_value !== null && response[0].my_value !== undefined ? parseFloat(response[0].my_value) : null;
    ...
    /**
     * Get total value of all points in USD (including pending points)
    ...
    async getTotalValue() {
      ...
      return this.entries.reduce((total, entry) => {
        ...
        return total + (totalAmount * entry.pointValue);
      }, 0);
    }
