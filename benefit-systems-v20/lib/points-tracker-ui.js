...
(function() {
  ...
  class PointsTrackerUI {
    constructor() {
      this.container = null;
      this.editingId = null;
      this.currentSort = { field: 'myValue', direction: 'desc' }; // Default: sort by My Value descending
      this.filters = {
        search: '',
        program: '',
    ...
    init() {
      ...
      const checkSection = () => {
        ...
        // If section not found after max attempts, exit silently (Information section may have been removed from UI)
      };
      checkSection();
    }
    ...
    async render() {
      ...
      try {
        ...
        const currentPoints = allEntries.reduce((total, entry) => {
          return total + (parseFloat(entry.points) || 0);
        }, 0);
        
        const pendingPoints = allEntries.reduce((total, entry) => {
          return total + (parseFloat(entry.pointsPending) || 0);
        }, 0);
        
        // Calculate total value (combined)
        ...
        // Calculate factor: My Total Value / Total Value
        const factor = totalValue > 0 ? totalMyValue / totalValue : 0;
        
        logger.info(`Current Points: ${currentPoints}, Pending Points: ${pendingPoints}, Total Value: $${totalValue}, Total My Value: $${totalMyValue}, Factor: ${factor.toFixed(2)}x`);
        ...
        if (statsDiv) {
          ...
          const currentPointsFormatted = currentPoints.toLocaleString('en-US', {
            maximumFractionDigits: 0
          });
          
          const pendingPointsFormatted = pendingPoints.toLocaleString('en-US', {
            maximumFractionDigits: 0
          });
          
          statsDiv.innerHTML = `
            <div class="points-stat-item">
              <span class="points-stat-label">Current Points:</span>
              <span class="points-stat-value">${currentPointsFormatted}</span>
            </div>
            <div class="points-stat-item">
              <span class="points-stat-label">Pending Points:</span>
              <span class="points-stat-value">${pendingPointsFormatted}</span>
            </div>
            <div class="points-stat-item">
              <span class="points-stat-label">Average Value:</span>
              <span class="points-stat-value">$${Math.round(totalValue).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')}</span>
            </div>
            <div class="points-stat-item">
              <span class="points-stat-label"><strong>My Value:</strong></span>
              <span class="points-stat-value"><strong>$${Math.round(totalMyValue).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')}</strong></span>
            </div>
            <div class="points-stat-item">
              <span class="points-stat-label">Factor:</span>
              <span class="points-stat-value">${factor.toFixed(1)}x</span>
            </div>
          `;
        }
    ...
    renderListView(sortedEntries, allEntries) {
      ...
      const sortField = this.currentSort.field || 'myValue';
      ...
      table.innerHTML = `
        <thead>
          <tr>
            ...
            <th class="points-sortable" data-sort="myValuePointValue">
              <button class="points-sort-button">
                My Value (¢)
                <span class="points-sort-indicator ${sortField === 'myValuePointValue' ? 'active' : ''}">${sortField === 'myValuePointValue' ? sortIndicator : ''}</span>
              </button>
            </th>
            <th class="points-sortable" data-sort="factor">
              <button class="points-sort-button">
                Faktor
                <span class="points-sort-indicator ${sortField === 'factor' ? 'active' : ''}">${sortField === 'factor' ? sortIndicator : ''}</span>
              </button>
            </th>
    ...
    createSimpleTableRow(entry) {
      try {
        ...
        // Get my_value point value from Supabase (per-point value)
        const myValuePointValue = entry.myValue !== null && entry.myValue !== undefined 
          ? parseFloat(entry.myValue) 
          : pointValue;
        // Use my_value from Supabase multiplied by total amount of points for My Value
        const myValue = myValuePointValue * totalAmount;
        // Calculate factor: My Value / Total Value
        const factor = totalValue > 0 ? myValue / totalValue : 0;
        // Convert my_value point value to cents for CPM display
        const myValueCents = myValuePointValue * 100;
        ...
        return `
          <tr class="${rowClass}">
            ...
            <td class="points-my-value" style="font-weight: bold; color: black; font-size: inherit;"><strong>$${Math.round(myValue).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')}</strong></td>
            <td class="points-my-value-cents">${myValueCents.toFixed(2)} ¢</td>
            <td class="points-factor">${factor.toFixed(1)}x</td>
    ...
    bindEvents() {
      ...
      if (resetBtn) {
        resetBtn.addEventListener('click', () => {
          ...
          this.currentSort = { field: 'myValue', direction: 'desc' };
          this.render();
        });
      }
    ...
    applySorting(entries) {
      ...
      const sorted = [...entries].sort((a, b) => {
        ...
        switch (this.currentSort.field) {
          ...
          case 'myValuePointValue':
            // Sort by my_value point value (per-point value from Supabase)
            const aMyValuePointValue = a.myValue !== null && a.myValue !== undefined 
              ? parseFloat(a.myValue) 
              : a.pointValue;
            const bMyValuePointValue = b.myValue !== null && b.myValue !== undefined 
              ? parseFloat(b.myValue) 
              : b.pointValue;
            aValue = aMyValuePointValue;
            bValue = bMyValuePointValue;
            return aValue - bValue;

          case 'factor':
            // Calculate factor for each entry: My Value / Total Value
            const aTotalForFactor = (parseFloat(a.points) || 0) + (parseFloat(a.pointsPending) || 0);
            const bTotalForFactor = (parseFloat(b.points) || 0) + (parseFloat(b.pointsPending) || 0);
            const aMyValue = a.myValue !== null && a.myValue !== undefined 
              ? parseFloat(a.myValue) * aTotalForFactor
              : aTotalForFactor * a.pointValue;
            const bMyValue = b.myValue !== null && b.myValue !== undefined 
              ? parseFloat(b.myValue) * bTotalForFactor
              : bTotalForFactor * b.pointValue;
            const aTotalValue = aTotalForFactor * a.pointValue;
            const bTotalValue = bTotalForFactor * b.pointValue;
            aValue = aTotalValue > 0 ? aMyValue / aTotalValue : 0;
            bValue = bTotalValue > 0 ? bMyValue / bTotalValue : 0;
            return aValue - bValue;
  ...
  // Initialize Points Tracker UI (only when Information section exists in the DOM)
  function initializePointsTrackerUI() {
    const initUI = () => {
      // Skip entirely if Information section is not in the UI
      if (!document.getElementById('bs-information-section')) {
        return;
      }
