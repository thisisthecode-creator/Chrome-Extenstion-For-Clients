...
function shouldAutoScrollToAwardSection() {
  try {
    const url = new URL(window.location.href)
    return url.searchParams.get('bs-scroll-award') === '1' || sessionStorage.getItem('bs-scroll-award') === '1'
  } catch (_) {
    try { return sessionStorage.getItem('bs-scroll-award') === '1' } catch (_) { return false }
  }
}

function clearAutoScrollToAwardFlag() {
  try { sessionStorage.removeItem('bs-scroll-award') } catch (_) {}
  try {
    const url = new URL(window.location.href)
    if (url.searchParams.has('bs-scroll-award')) {
      url.searchParams.delete('bs-scroll-award')
...
function readFlightInputs() {
  ...
  if (!from || !to || !depart) {
    try {
      ...
      // Extra robust parsing for mobile and new Google Flights URLs
      // Reuse the same logic as extractAirportsFromURL in content.js
      if (!from || !to) {
        ...
        // Try parsing from pathname: /WAW-VIE/
...
function createStandaloneAwardSection() {
  ...
  if (!targetElement) {
    // Mobile or changed Google Flights layout: fall back to a generic main container
    console.log('Target element for Award Flight Analysis not found with primary selectors, using fallback container')
    targetElement =
      document.querySelector('div[role="main"]') ||
      document.querySelector('main') ||
      document.querySelector('div[jsname="dSdcbd"]') || // generic GF main content wrapper (best-effort)
      document.body
  }
  ...
  try {
    ...
    function waitForReady() {
      if (isPageReady()) {
        ...
        injectFlightDetails()
        // After injection, trigger Award Flight Analysis + Seats.aero data and then fill price.
        // This makes sure the 🏆 Award Flight Analysis section also appears on mobile layouts.
        setTimeout(() => {
          try {
            ...
            console.warn("[BS Extension] triggerGlobalPanelUpdate failed:", e)
          }
