...
// Fetches and manages airport data from OpenFlights + airportsdata
...
class AirportDataService {
  ...
  // Fetch airport data from OpenFlights + airportsdata
  ...
  async _loadAirportData() {
    try {
      console.log('Fetching airport data from OpenFlights + airportsdata...');
      const [openFlightsResult, airportsDataResult] = await Promise.allSettled([
        fetch('https://raw.githubusercontent.com/jpatokal/openflights/master/data/airports.dat'),
        fetch('https://raw.githubusercontent.com/mborsetti/airportsdata/refs/heads/main/airportsdata/airports.csv')
      ]);

      const openFlightsAirports = await this._parseFetchResult(openFlightsResult, 'OpenFlights');
      const airportsDataAirports = await this._parseFetchResult(airportsDataResult, 'airportsdata');

      this.airports = this._mergeAirportLists(openFlightsAirports, airportsDataAirports);
      this.isLoaded = true;
  ...
  async _parseFetchResult(fetchResult, sourceName) {
    if (fetchResult.status !== 'fulfilled') {
      console.warn(`${sourceName} fetch failed:`, fetchResult.reason);
      return [];
    }

    const response = fetchResult.value;
    if (!response.ok) {
      console.warn(`${sourceName} returned HTTP ${response.status}`);
      return [];
    }
  ...
  _mergeAirportLists(openFlightsAirports, airportsDataAirports) {
    const airportsByIata = new Map();

    // Start with OpenFlights data.
    for (const airport of openFlightsAirports) {
      airportsByIata.set(airport.iata, airport);
    }

    // Overwrite with airportsdata for fresher records.
    for (const airport of airportsDataAirports) {
      airportsByIata.set(airport.iata, airport);
    }

    return Array.from(airportsByIata.values());
  }

  // Parse CSV data from OpenFlights/airportsdata format
  _parseCSV(csvText) {
