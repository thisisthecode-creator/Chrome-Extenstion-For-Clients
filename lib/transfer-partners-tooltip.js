    ...
    <div class="selected-cards-comparison">
      <div class="comparison-header">
        ...
        <div class="display-toggle">
          ...
          <button class="toggle-btn" data-display="both">Both</button>
        </div>
      </div>
      ...
      ${selectedCardsData.map(card => {
        const getMultiplierDisplay = (value, category, displayType = 'points') => {
          if (!value || value === 0) {
            const defaultText = displayType === 'cashback' ? '2.71%' : 
                               displayType === 'both' ? '1x • 2.71%' : '1x • 2.71%';
    ...
    button.addEventListener('click', function() {
      ...
      multiplierCells.forEach(cell => {
        ...
        if (multiplierMatch) {
          ...
          const percentage = (multiplier * 2.71).toFixed(2) + '%';
          
          if (displayType === 'cashback') {
            cell.textContent = percentage;
          } else if (displayType === 'both') {
            cell.textContent = `${multiplier}x • ${percentage}`;
          } else {
    ...
    <div class="transfer-partners-comparison">
      <div class="comparison-header">
        <div class="buttons-row">
          ...
          <div class="display-toggle">
            <button class="toggle-btn active" data-display="ratio">Ratio</button>
            <button class="toggle-btn" data-display="percentage">%</button>
          </div>
        </div>
      </div>
