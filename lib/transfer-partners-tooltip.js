...
// Function to preselect specific cards by name
function preselectDefaultCards() {
  ...
  if (window.allCards && window.allCards.length > 0) {
    defaultCardNames.forEach(cardName => {
      const card = window.allCards.find(c => c.name === cardName);
      if (card) {
        window.selectedCards.add(card.id.toString());
        console.log(`Preselected card: ${cardName} (ID: ${card.id})`);
      } else {
        console.warn(`Card not found: ${cardName}`);
      }
    });
    
    console.log('Preselected cards:', Array.from(window.selectedCards));
  }
}
...
async function loadCombinedData() {
  try {
    ...
    // Preselect default cards
    preselectDefaultCards();
  ...
  // Apply default filters for Available Transfer Partners (Amex, Chase, Cap1, % view)
  setTimeout(() => {
    filterTransferPartnersByPrograms();
    applyDefaultDisplayType();
  }, 100);
  ...
  try {
    ...
    if (savedData) {
      ...
      if (informationData.selectedCards && Array.isArray(informationData.selectedCards)) {
        ...
        // No saved cards, keep preselected cards
        console.log('No saved cards found, keeping preselected cards:', Array.from(window.selectedCards));
      }
...
function renderAvailableTransferPartners() {
      ...
      <div class="comparison-header">
        <div class="buttons-row">
          <div class="credit-card-filters">
            ...
            <button class="filter-btn" data-program="citi">Citi</button>
            <button class="filter-btn active" data-program="capital-one">Cap1</button>
            <button class="filter-btn" data-program="bilt">Bilt</button>
            <button class="filter-btn" data-program="wells-fargo">Wells Fargo</button>
          </div>
          <div class="display-toggle">
  ...
  toggleButtons.forEach(button => {
    button.addEventListener('click', function() {
      ...
      // Update all ratio values in the table
      const ratioCells = document.querySelectorAll('.transfer-partners-comparison .ratio-value');
      ratioCells.forEach(cell => {
        const originalValue = cell.getAttribute('data-original-value') || cell.textContent;
        if (!cell.getAttribute('data-original-value')) {
          cell.setAttribute('data-original-value', originalValue);
        }
        ...
        if (originalValue === '✗') {
          ...
          const ratioMatch = originalValue.match(/(\d+(?:\.\d+)?):(\d+(?:\.\d+)?)/);
          if (ratioMatch) {
            const from = parseFloat(ratioMatch[1]);
            const to = parseFloat(ratioMatch[2]);
            const percentage = Math.round((to / from) * 100);
...
// Apply default program filters (Amex, Chase, Cap1)
function filterTransferPartnersByPrograms() {
  const defaultActivePrograms = ['amex', 'chase', 'capital-one'];
  const inactivePrograms = ['citi', 'bilt', 'wells-fargo'];
  
  // Hide inactive program columns
  inactivePrograms.forEach(program => {
    ...
    const filterBtn = document.querySelector(`[data-program="${program}"]`);
    if (filterBtn) {
      filterBtn.classList.remove('active');
    }
  });
  
  console.log('Applied default program filters:', defaultActivePrograms);
}

// Apply default display type (percentage)
function applyDefaultDisplayType() {
  const toggleButtons = document.querySelectorAll('.transfer-partners-comparison .toggle-btn');
  const percentageButton = document.querySelector('.transfer-partners-comparison [data-display="percentage"]');
  const ratioButton = document.querySelector('.transfer-partners-comparison [data-display="ratio"]');
  
  if (percentageButton && ratioButton) {
    // Remove active from ratio button
    ratioButton.classList.remove('active');
    // Add active to percentage button
    percentageButton.classList.add('active');
    
    // Apply percentage display to all ratio cells
    const ratioCells = document.querySelectorAll('.transfer-partners-comparison .ratio-value');
    ratioCells.forEach(cell => {
      const originalValue = cell.getAttribute('data-original-value') || cell.textContent;
      if (!cell.getAttribute('data-original-value')) {
      ...
      if (originalValue === '✗') {
        cell.innerHTML = '<span class="no-value">✗</span>';
      } else if (originalValue !== 'N/A' && originalValue !== '') {
        // Convert ratio to percentage (assuming 1:1 ratio = 1%)
        const ratio = parseFloat(originalValue);
        if (!isNaN(ratio)) {
    ...
    console.log('Applied default display type: percentage');
  }
}
