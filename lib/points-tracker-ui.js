...
(function() {
  ...
  class PointsTrackerUI {
    constructor() {
      this.container = null;
      this.editingId = null;
      this.currentSort = { field: 'amount', direction: 'desc' }; // Default: sort by amount (points) descending
      this.filters = {
        search: '',
        program: '',
        ...
        <div class="points-modal" id="points-modal" style="display: none;">
          <div class="points-modal-content">
            ...
            <form id="points-form">
              ...
              <div class="points-form-group">
                <label for="points-custom-point-value">Custom Point Value (optional)</label>
                <input type="number" id="points-custom-point-value" step="0.0001" min="0" placeholder="Auto (uses program default)">
                <small style="color: #5f6368; font-size: 12px; display: block; margin-top: 4px;">Leave empty to use program default. Value in dollars (e.g., 0.0190 = 1.9¢ per point)</small>
              </div>
              <div class="points-form-group">
    ...
    renderListView(sortedEntries, allEntries) {
      ...
      const sortField = this.currentSort.field || 'amount';
        ...
        <thead>
          <tr>
            ...
            <th class="points-sortable" data-sort="amount">
              <button class="points-sort-button">
                Amount
                <span class="points-sort-indicator ${sortField === 'amount' ? 'active' : ''}">${sortField === 'amount' ? sortIndicator : ''}</span>
              </button>
            </th>
    ...
    createSimpleTableRow(entry) {
      try {
        ...
        const pending = parseFloat(entry.pending) || 0;
        const totalAmount = amount + pending;
        const defaultPointValue = parseFloat(entry.pointValue) || 0;
        // Use custom point value if set, otherwise use default
        const customPointValue = entry.customPointValue !== null && entry.customPointValue !== undefined 
          ? parseFloat(entry.customPointValue) 
          : defaultPointValue;
        const myValue = totalAmount * customPointValue;
        const totalValue = totalAmount * defaultPointValue;
        ...
        return `
          <tr class="${rowClass}">
            ...
            <td class="points-amount">${amount.toLocaleString()}</td>
            <td class="points-pending">${pending > 0 ? pending.toLocaleString() : '-'}</td>
    ...
    createProgramGroup(programName, entries) {
      ...
      const totalValue = entries.reduce((sum, e) => sum + ((parseFloat(e.amount) || 0) * (parseFloat(e.pointValue) || 0)), 0);
    ...
    createEntryCard(entry) {
      try {
        ...
        const daysLeft = entry.expirationDate ? window.pointsTracker.getDaysUntilExpiration(entry.expirationDate) : null;
        const totalValue = amount * pointValue;
        const isExpired = daysLeft !== null && daysLeft < 0;
        const isExpiringSoon = daysLeft !== null && daysLeft >= 0 && daysLeft <= 90;
          ...
          <div class="points-entry-card ${statusClass}" data-entry-id="${entry.id}">
            <div class="points-card-header">
              <div class="points-card-amount">
                <span class="points-card-amount-value">${amount.toLocaleString()}</span>
    ...
    bindEvents() {
      ...
      if (resetBtn) {
        resetBtn.addEventListener('click', () => {
          ...
          this.currentSort = { field: 'amount', direction: 'desc' };
          this.render();
        });
      }
    ...
    async openModal(entryId = null) {
      ...
      if (entryId) {
        ...
        if (entry) {
          ...
          document.getElementById('points-pending').value = entry.pending || '';
          document.getElementById('points-custom-point-value').value = entry.customPointValue || '';
    ...
    async saveEntry() {
      ...
      const customPointValueInput = document.getElementById('points-custom-point-value').value;
      const customPointValue = customPointValueInput && customPointValueInput.trim() !== '' 
        ? customPointValueInput.trim() 
        : null;
      ...
      try {
        if (this.editingId) {
          ...
          await window.pointsTracker.updateEntry(this.editingId, {
            programName: program,
            amount: amount,
            pending: pending && pending.trim() !== '' ? pending.trim() : null,
    ...
    applySorting(entries) {
      ...
      const sorted = [...entries].sort((a, b) => {
        ...
        switch (this.currentSort.field) {
          ...
          case 'pending':
            aValue = parseFloat(a.pending) || 0;
            bValue = parseFloat(b.pending) || 0;
            return aValue - bValue;

          case 'totalAmount':
            aValue = (parseFloat(a.amount) || 0) + (parseFloat(a.pending) || 0);
            bValue = (parseFloat(b.amount) || 0) + (parseFloat(b.pending) || 0);
            return aValue - bValue;

          case 'myValue':
            const aTotalAmount = (parseFloat(a.amount) || 0) + (parseFloat(a.pending) || 0);
            const bTotalAmount = (parseFloat(b.amount) || 0) + (parseFloat(b.pending) || 0);
            const aCustomValue = parseFloat(a.customPointValue) || a.pointValue;
            const bCustomValue = parseFloat(b.customPointValue) || b.pointValue;
            aValue = aTotalAmount * aCustomValue;
            bValue = bTotalAmount * bCustomValue;
            return aValue - bValue;

          case 'totalValue':
            const aTotal = (parseFloat(a.amount) || 0) + (parseFloat(a.pending) || 0);
            const bTotal = (parseFloat(b.amount) || 0) + (parseFloat(b.pending) || 0);
